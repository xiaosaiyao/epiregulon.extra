...
findDifferentialActivity <- function(activity_matrix,
  ...
  tf_markers <- findMarkersSimple(activity_matrix, 
                                  clusters, 
                                  pval.type=pval.type, 
                                  direction=direction)
...
#' @param fdr_cutoff A numeric scalar to specify the cutoff for log FDR value. Default is log(0.05)
...
#' rownames(score.combine) <- paste0("TF",1:20)
#' colnames(score.combine) <- paste0("cell",1:200)
#' clusters <- c(rep(1,100), rep(2,100))
#' markers <- findDifferentialActivity(score.combine, clusters, pval.type="some", direction="up")
#' sig.genes <- getSigGenes(markers, fdr_cutoff=1, summary_cutoff=0.1)
...
getSigGenes <- function(da_list,
  ...
  top.list <- lapply(seq_along(da_list), function(i){
    ...
    summary.name <- colnames(da_genes)[grep("logFC|diff", colnames(da_genes))]
    da_genes <- da_genes[,c(p.value.name, FDR.name,  summary.name)]
    
    if (is.null(summary_cutoff)){
      
      if (direction %in% c("up","down")) {
        summary_cutoff <- signif(quantile(da_genes[, summary.name]*direction_factor, 0.95, na.rm=TRUE), digits=2 )
      } else{
        summary_cutoff <- signif(quantile(abs(da_genes[, summary.name]), 0.95, na.rm=TRUE), digits=2)
      }
    }else {
    ...
    if (is.null(topgenes)){
      ...
      da_genes <- da_genes[head(order(da_genes[, FDR.name], -(da_genes[, 3])), topgenes),]
    }
...
regulonEnrich_ <- function(TF, regulon, corr, corr_cutoff, genesets) {
  ...
  if (length(intersect(regulon.TF, genesets$genes)) < 3 | all(!genesets$genes %in% regulon.TF)) {
    ...
    enrichresults <- enricher(regulon.TF, TERM2GENE=genesets)
