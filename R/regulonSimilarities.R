...
findPartners <- function(graph, focal_tf) {
  ...
  if (!is(focal_tf, "character")) {
    stop("focal_tf must be a charactor")
  }
  
  if (length(unique(vertex_attr(graph, name="type"))) != 2) {
    stop("Graph should contain vertices of two types. \n
         Input graph should not be created with the use of tripartite mode")
  }
