...
findPartners <- function(graph, focal_tf) {
    ...
    tf_targets_subgraphs <- lapply(all_tfs, function(tf, targets) igraph::subgraph(graph,
        ...
        focal_tf_targets)
    res_list <- list()
    focal_weights <- igraph::get.edge.attribute(tf_targets_subgraphs[[1]], index = E(tf_targets_subgraphs[[1]]),
        name = "weight")
    for (i in seq_along(all_tfs)) {
        # skip focal tf
        if (i == 1)
            next
        tf <- V(tf_targets_subgraphs[[i]])[V(tf_targets_subgraphs[[i]])$type == "transcription factor" &
            V(tf_targets_subgraphs[[i]])$name == all_tfs[i]$name]
        # determine targets shared between focal tf and current tf
        common_targets <- V(tf_targets_subgraphs[[i]])[V(tf_targets_subgraphs[[i]])$type ==
            "target gene"]
        # extract edges connection shared targets with the current tf
        edges_to_tf <- shortest_paths(tf_targets_subgraphs[[i]], from = tf, to = common_targets,
            output = "epath")
        ...
        res_list[[tf$name]] <- data.frame(target = common_targets$name, focal_weight = focal_weights[common_targets_ind],
