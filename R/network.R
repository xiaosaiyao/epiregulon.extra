...
plotGseaNetwork <- function(tf, 
  ...
  enrichresults.filter <- lapply(setNames(names(enrichresults), names(enrichresults)),
                                 function(x) {
                                   enrichresults[[x]][, "p.adjust"] <- as.numeric(enrichresults[[x]][, "p.adjust"])
                                   enrichresults[[x]][which(enrichresults[[x]][, "p.adjust"] < p.adj_cutoff), ]
                                 })
  
  # extract top pathways from enrichr results
  pathway.list <- lapply(setNames(tf, tf), function(x) {
    enrichresults[[x]][, "p.adjust"] <- as.numeric(enrichresults[[x]][, "p.adjust"])
    head(enrichresults.filter[[x]][order(enrichresults.filter[[x]][,"p.adjust"]), ], ntop_pathways)
  })
  ...
  V(p)$type.num <- as.numeric(factor(V(p)$type, 
                                     levels=c(tf_label, gset_label)))
